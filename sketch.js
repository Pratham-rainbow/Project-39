...
function setup() {
  ...
  gameOver = createSprite(width/2, height-190);
...
function draw() {
  ...
  textSize(20);
  fill(0);
  text("Score: " + score, 30, trex.y-300);
  pop();
  ...
  if (gameState === PLAY) {
    ...
    if (trex.y > height-70 && keyDown("space") || touches.length>0) {
      trex.velocityY = -12;
      touches = [];
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(height/2-200, height/2+50));
